...
## What's actually shipped (PR #39)

All five queued items are now in the codebase. Use them as follows:

1. **`dataDelaySec` prop on `chart-scene`** — number of seconds to hold the chart's DATA layer (bars, lines, markers, pills) at opacity 0 while the chrome (title + subtitle + footer + gridlines + axis labels) plays its entrance. Set explicitly to pre-roll the chrome (e.g. `0.5` for the 0.5s pre-roll), or leave it at the default `0` and let audio-sync compute it for you. Clamped to `[0, 4]`. See `packages/core/src/script/templates/builtin.ts` (chart-scene template) for the implementation.

2. **`chart-payoff` template** — a 1.5–3s scene that displays the chart's annotation text big and centered, in the chart's theme. Same red-rule chrome as `chart-scene` so the pair reads as one beat. Auto-paired with every `chart-scene` by the planner heuristics — you don't typically author these by hand, but you can. See `packages/core/src/script/templates/builtin.ts` (CHART_PAYOFF).

3. **Planner heuristics** — `applyRetentionHeuristics()` runs at the end of `planScript`, applying four pure post-processors to the planned Script:
   - `avoidChartAtSceneOne` — if scene 0 is `chart-scene`, prepends a `hook-bigtext` with the chart's title.
   - `capConsecutiveCharts` — when 3+ chart-scenes appear in a row, inserts a `kinetic-words` bridge between the 2nd and 3rd.
   - `autoEmitPayoff` — adds a `chart-payoff` scene after every `chart-scene`, pulling the payoff text from the chart's annotation (or subtitle/title as fallback). Skipped if the planner already emitted a payoff.
   - `clampChartDurations` — short-form chart-scenes capped at 5s, long-form at 10s.

   See `packages/core/src/script/retentionHeuristics.ts` + tests at `retentionHeuristics.test.ts`.

4. **Format-aware sustain caps** — `inferScriptFormat(meta)` returns `"short"` or `"long"` based on `meta.format` or `meta.targetDurationSeconds` (≤90s → short). The clamp logic above uses this.

5. **Audio-sync** — `findChartLeadInTranscript()` matches a chart's prop values against word-level transcript timestamps. The assembler reads `<projectDir>/transcript.json` (produced by `hyperframes transcribe`) and overrides each chart-scene's `dataDelaySec` so the data layer rises with the spoken word minus a 250ms lead.

   Author-set `dataDelaySec` is preserved if larger than the auto-computed value (manual pre-roll is never shortened). When no transcript exists or no chart value matches the narration, audio-sync degrades silently to no-op. See `packages/core/src/script/audioSync.ts` + tests at `audioSync.test.ts`.

### How to use audio-sync end-to-end

```bash
# 1. Plan + synth as usual
hyperframes script ./script.md

# 2. Transcribe the synthesized audio to word-level timestamps
hyperframes transcribe ./voice.mp3 -d .

# 3. Re-assemble — the assembler picks up transcript.json automatically
hyperframes render
```

The render now has each chart-scene's data layer entering at the spoken word timestamp.
